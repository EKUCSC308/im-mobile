...
class Conversations {
    ...
    func createConvo(label: String, cb: @escaping (_ response: Convos?, _ error: String?) -> Void) throws {
        ...
        let task = URLSession.shared.dataTask(with: request) {(data, response, error) in
            if (error != nil) {
                ...
                do {
                    let createResponse = try JSONDecoder().decode(Convos.self, from: data!)
